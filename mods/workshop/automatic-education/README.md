# Automatic Education - Guardian & University Manager

- 标识：`automatic-education`；类型：`workshop`；版本：`2.1.0`；CK3：`1.19.*`。
- 功能：自动安排监护人、教育重心、大学及童子试。
- 前置 Mod：无（本地descriptor无声明；线上待复核）。
- 导入时间：2026-09-18T15:56:38+08:00。来源为本机已安装文件，不代表线上最新版本。
- 上游：https://steamcommunity.com/sharedfiles/filedetails/?id=3717349873（2.1.0）。
- 文件指纹：`sha256:dd2400650269c17ac9a3ecb37e3d35ee496d49626cb966e74f5017ff54ddd652`；明细见 references/source-files.json。

## 安装与加载顺序

游戏文件已按AGENTS.md部署至 `G:\SteamLibrary\steamapps\common\Crusader Kings III\mod\automatic-education`。此目录是额外本地副本，未改变当前用户mod目录或播放集。
